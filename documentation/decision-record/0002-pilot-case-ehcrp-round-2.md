...
| `Ambrose Memorial Park EHCRP.docx` / `Ambrose Center Park EHCRP.docx` | One **working document**, forked. ~15k words each and ~99% identical. They differ only in the pre-application section (title, 2a, Q5, 6a, one Modeling paragraph). Contents: NOFA summary, webinar notes, strategy reflections, partner brainstorm, meeting notes with action items, outreach email templates, a "Fresno EHCRP example", firm experience boilerplate, technical capabilities, local knowledge, and pre-app draft answers, including long and "SHORTENED VERSION" project descriptions. |
...
2. **Reuse happens by forking whole documents.** The two Ambrose files show it. We need near-duplicate detection and chunk-level lineage (`variant-of`, `shortened-from`, `adapted-for`).
