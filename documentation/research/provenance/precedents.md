...
- **Lesson for gentext:** the hierarchical *hash-then-diff* approach (paragraph → sentence → token) is exactly what our harvest step should do ([git blame and hashing](git-blame-and-hashing.md)). Token-level authorship depends on an unbroken chain of revisions with known editors. Our Google Docs paste breaks that chain, so declared provenance must fill the gap.
...
- **Lesson for gentext:** entry-level **owner + review cadence + tags** is the commercial baseline. Our frontmatter should match it (`owner`, `review.status/by/on`). Span provenance and AI generation records are where we would go beyond the commercial tools.
