...
> **TL;DR** **Adopt** as the M0 DOCX→Markdown converter. It is deterministic, fast, and ships as a single binary. Its JSON AST is fully inspectable, and it is the only tool we tested that keeps Word comments and suggestions. Use `-f docx+styles --track-changes=all` with a Lua filter for gentext-specific cleanup.
