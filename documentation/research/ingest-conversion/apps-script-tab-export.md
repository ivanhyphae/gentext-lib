...
> **TL;DR** **Trial.** This is a small bound or standalone Apps Script that walks `getTabs()`/`getChildTabs()` and writes **one JSON (or Markdown) file per tab** to a Drive folder. Each file carries the tab id, title, path and paragraph heading levels. It is the lowest-friction way for non-developers to hand gentext a tab-faithful snapshot without an OAuth app.
...
- The team works in Google Docs, and a custom menu item ("Export to gentext") fits their workflow.

## How it would fit
Recommended output: `{docId, revisionId?, exportedAt, tabs:[{tabId, title, path, paragraphs:[{heading, text, listLevel}]}]}` as JSON, not home-made Markdown. gentext then renders Markdown deterministically in Python. The JSON is the inspectable artifact that gets committed to the derived area.
